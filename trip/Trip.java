...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        int i = 1;
        Road curRoad = _map.getLabel(segment.get(0), segment.get(1));
        double distance = curRoad.length();
        Location dest = _map.getLabel(segment.get(segment.size() - 1));
        while (i < segment.size() - 1) {
            Road road = _map.getLabel(segment.get(i), segment.get(i + 1));
            if (road.toString().equals(curRoad.toString())
                    && road.direction().equals(curRoad.direction())) {
                distance += road.length();
            } else {
                System.out.println(seq + ". Take " + curRoad + " "
                        + curRoad.direction().fullName() + " for "
                        + format.format(distance)  + " miles.");
                curRoad = road;
        ...
        System.out.println(seq + ". Take " + curRoad + " "
                + curRoad.direction().fullName() + " for "
                + format.format(distance) + " miles to " + dest + ".");
        return seq + 1;
    }
